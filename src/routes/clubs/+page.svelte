...
<script lang="ts">
  ...
  const clubs = clubTypes as unknown as ClubType[]; // Using unknown to bypass the "vibe" error
  ...
  $: filteredClubs = clubs
    ...
    .filter(c => showLounge ? c.is_lounge === true : true) // New
    .filter(b => {
...
<div class="min-h-screen bg-purple-50">
  ...
  <p class="text-sm text-vibe-brown/70 italic">
    Explore nightlife scenes, energy levels, <strong>dance</strong>-forward, and <strong>fluid</strong> environments.
  </p>
      ...
      {#each filteredClubs as club}
        ...
        >
          ...
          <div class="mt-2 flex flex-wrap gap-2">
            ...
            {#if club.is_fluid}
              <span class="text-[10px] font-bold uppercase tracking-wider bg-blue-50 text-blue-700 px-2 py-0.5 rounded-full border border-blue-100">
                Fluid
              </span>
            {/if}
            ...
            {#if club.global_availability}
              ...
              >
                {club.global_availability === 'widespread' ? '🌎 Common' : club.global_availability === 'regional' ? '🗺️ Regional' : '✨ Rare'}
              </span>
            {/if}
          </div>
        </a>
